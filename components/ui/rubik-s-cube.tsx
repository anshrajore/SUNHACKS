...
const RubiksCubeModel = forwardRef(function RubiksCubeModel(props, ref) {
  ...
  const currentMoveRef = useRef<{ axis: string; layer: number; direction: number; rotationAngle?: number } | null>(null);
  ...
  const easeInOutQuad = useCallback((t: any) => {
    return t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
  }, []);

  const matrixToQuaternion = useCallback((matrix: any) => {
  ...
  const updateCubes = useCallback((prevCubes: Cube[], move: any, stepRotationMatrix: any) => {
...
function EnhancedSpotlight(props: any) {
  ...
  useEffect(() => {
    if (light.current) {
      (light.current as any).target.position.set(0, 0, 0);
